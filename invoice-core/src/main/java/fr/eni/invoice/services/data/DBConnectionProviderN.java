 ...
 *
 */
public class DBConnectionProviderN {
	
	//TODO rendre cette taille configurable
	private static final int TAILLE_POOL = 3;

	private static DBConnectionProviderN provider;
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(DBConnectionProviderN.class); 
	
	
	
	private DBConnectionProviderN()  {
	 ...
	 */
	public static DBConnectionProviderN getInstance() {
		if (provider == null) {
			provider = new DBConnectionProviderN();
		}
		return provider;
	}
	...
	public Connection acquire() throws InterruptedException {
		...
		LOGGER.logDebug("connexion prise, reste " + connectionSemaphore.availablePermits() + " connexion(s), " + connectionSemaphore.getQueueLength() + " threads bloqué(s)" );
	...
	public void release(Connection connection) {
		...
		LOGGER.logDebug("connexion relâchée, reste " + connectionSemaphore.availablePermits() + " connexion(s) - thread id = " + ThreadContext.getContext("thread-id"));
